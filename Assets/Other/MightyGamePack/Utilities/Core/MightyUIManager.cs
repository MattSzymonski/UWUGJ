...
namespace MightyGamePack
{
    ...
    public class MightyUIManager : MonoBehaviour
    {
        ...
        {
            if ((animatedCameraInMM && gameManager.gameState == GameState.MainMenu) || (animatedCameraInGOM && gameManager.gameState == GameState.GameOverMenu) || (gameManager.gameState == GameState.OptionsMenu && gameManager.previousGameState == GameState.MainMenu))
            {
                if (!blockUIInput)
                {
        ...
        {
            ToggleUIInputBlock(true);
            if (gameManager.previousGameState == GameState.PauseMenu)
            {
                optionsMenuAnimator.SetBool("Opened", false);
                yield return new WaitForSeconds(GetAnimationFromAnimator(optionsMenuAnimator, GetAnimationNameFromUIIndex("OptionsMenuClose")).length);
                pauseMenuAnimator.SetBool("Opened", true);
            ...
            if (gameManager.previousGameState == GameState.MainMenu)
            {
                optionsMenuAnimator.SetBool("Opened", false);
                yield return new WaitForSeconds(GetAnimationFromAnimator(optionsMenuAnimator, GetAnimationNameFromUIIndex("OptionsMenuClose")).length);
                mainMenuAnimator.SetBool("Opened", true);
